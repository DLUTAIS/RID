...
from .shufflenetv2_new1 import shufflenetv2
...
class ABF(nn.Module):
    ...
    def forward(self, x, y=None, shape=None, out_shape=None):
        ...
        if self.att_conv is not None:
            ...
            y = F.interpolate(y, (x.shape[2], x.shape[3]), mode="nearest")
            # fusion
            # print(x.shape)
            # print(y.shape)
...
def build_review_kd(model, num_classes, teacher = ''):
    ...
    if 'x4' in model: 
        ...
        # student = shufflenetv2(class_num = num_classes)
        in_channels = [116,232,464,1024]
        shapes = [1,4,8,16]
        # out_channels = [64,128,256,256] # ?
        # out_shapes = [1,8,16,32]    # ³õÊ¼
        out_channels = [116,232,464,1024] # ?
        out_shapes = [1,28,56,112]    # ?
        #out_channels = [116,232,464,1024] # ?
        #out_shapes = [1,4,8,16]    # ?
    elif 'wrn' in model:
